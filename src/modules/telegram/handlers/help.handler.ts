      ...
      try {
        ...
        bot.once('message', async (replyMsg) => {
          ...
          try {
            ...
            await this.telegramService.sendMessage(adminTelegramId, `Yordam so‘rovi:\nFoydalanuvchi: ${replyMsg.from.id} (@${replyMsg.from.username || 'N/A'})\nXabar: ${replyText}`);
            await this.telegramService.sendMessage(chatId, `Sizning xabaringiz adminga (@${adminTelegramUser}) yuborildi. Tez orada javob olasiz!`);
          } catch (error) {
            this.logger.error(`Error sending help to admin: ${error.message}`);
            if (error.response?.body?.error_code === 403) {
              await this.telegramService.sendMessage(chatId, `Xabar yuborishda xato: Admin (@${adminTelegramUser}) bot bilan chat boshlamagan. Iltimos, @${adminTelegramUser} ga yozing.`);
            } else {
              await this.telegramService.sendMessage(chatId, `Xabar yuborishda xato: ${error.message}. Iltimos, @${adminTelegramUser} ga yozing.`);
            }
          }
        });
      } catch (error) {
        this.logger.error(`Error in help: ${error.message}`);
        await this.telegramService.sendMessage(chatId, `Yordam so‘rovida xato yuz berdi. Iltimos, @${adminTelegramUser} ga yozing.`);
      }
    });
  }
}